...
export default function RoadmapSection() {
  ...
  return (
    <section className="relative py-24 px-4 overflow-hidden bg-gradient-to-b from-black via-[#F2ECC8]/5 to-black">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-1 md:w-1.5 bg-gradient-to-b from-accent via-accent/60 to-accent/30 shadow-[#F2ECC8]"></div>
            ...
            {phases.map((phase, index) => (
              ...
              >
                ...
                <div className={`w-full md:w-5/12 ${index % 2 === 0 ? 'md:text-right' : 'md:text-left'} text-left`}>
                  <div className="bg-gradient-to-r from-[#F2ECC8]/20 via-[#F2ECC8]/10 to-[#F2ECC8]/20 border border-[#F2ECC8]/30 backdrop-blur-sm p-4 sm:p-6 transition-all duration-300 hover:border-[#F2ECC8]/50 hover:bg-[#F2ECC8]/25 shadow-[#a5a082] hover:shadow-[#a5a082] group">
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`${phase.color} px-3 py-1 text-xs font-bold text-[#F2ECC8]`}>
                        {phase.status}
                      </div>
                      <span className="text-sm text-accent font-semibold">{phase.phase}</span>
                    </div>
                    <h3 className="text-xl sm:text-2xl font-bold text-white mb-2 sm:mb-3 group-hover:text-[#F2ECC8] transition-colors duration-300">
                      {phase.title}
                    </h3>
                ...
                <div className="absolute left-4 md:left-1/2 md:-translate-x-1/2 w-10 h-10 md:w-14 md:h-14 bg-accent rounded-full flex items-center justify-center border-4 md:border-[6px] border-[#F2ECC8] shadow-[#F2ECC8] z-10 hover:scale-110 transition-transform duration-300">
                  <phase.icon className="text-[#F2ECC8]" size={24} />
                </div>

                {/* Connecting line from icon to card - More visible */}
                <div className={`absolute ${index % 2 === 0 ? 'md:right-1/2 md:mr-7' : 'md:left-1/2 md:ml-7'} left-14 w-6 md:w-[calc(50%-3.5rem)] h-0.5 md:h-1 bg-accent/40 top-1/2 -translate-y-1/2 hidden md:block shadow-[#4e4b3a]`}></div>
